...
Taking a look at the datasets (KB)
```{r}
str(boston)
str(berlin)
str(nyc)
...
Standardize all the marathon datasets (KB)
```{r}
boston_clean <- boston %>%
  transmute(
    year = year,
...
Standardize airquality data (KB)
```{r}
...
Convert the cleaned chip_time values to H:M:S and get chip_seconds (KB)
```{r}
marathons_all <- marathons_all %>%
  mutate(
...
Remove missing finish times. This is safe because we only use average finishing times in our model, so individual missing times don not matter. Also there are only 3 total. (KB)
```{r}
marathons_all <- marathons_all %>%
  filter(!is.na(chip_seconds))
```

Labeling Gender as 'male', 'female', or 'unknown', nonbinary falls under female (KB)
```{r}
...
Removing unknowns since we have only 29 unknowns, and they will not help the model and most likely add noise: (KB)
```{r}
marathons_all <- marathons_all %>% 
...
Creating time_ratio and using histogram to create subgroups (KB)
```{r}
