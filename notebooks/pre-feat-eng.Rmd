...
main_data <- main_data %>%
  mutate(
    pm25_imp  = factor(pm25_imp, levels = c(FALSE, TRUE)),
...
# Select numeric columns only (exclude factors like gender, subgroup, supershoe, pm25_imp)
numeric_vars <- main_data %>%
  select(where(is.numeric))

# Fit a linear model with all numeric predictors
vif_model <- lm(avg_chip_seconds ~ ., data = numeric_vars)

# Compute VIFs
vif_values <- vif(vif_model)

# Sort and view
vif_values_sorted <- sort(vif_values, decreasing = TRUE)
vif_values_sorted
