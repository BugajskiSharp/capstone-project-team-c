...
# Testing metrics
rmse_xgb <- rmse(test_data$avg_chip_seconds, pred_test_xgb)
mae_xgb  <- mae(test_data$avg_chip_seconds, pred_test_xgb)
...
rss_test <- sum((test_data$avg_chip_seconds - pred_xgb)^2)  # Residual sum of squares
...
```{r}
# Align marathon
berlin_data$marathon <- factor(
  berlin_data$marathon,
  levels = levels(train_data$marathon)  # 3 levels from training 
)

# Align gender
berlin_data$gender <- factor(
  berlin_data$gender,
  levels = levels(train_data$gender)
)

# Align subgroup
berlin_data$subgroup <- factor(
  berlin_data$subgroup,
  levels = levels(train_data$subgroup)
)

# Align supershoe
berlin_data$supershoe <- factor(
  berlin_data$supershoe,
  levels = levels(train_data$supershoe)
)

# Align ozone_bin
berlin_data$ozone_bin <- factor(
  berlin_data$ozone_bin,
  levels = levels(train_data$ozone_bin)
)

# Align pm25_bin
berlin_data$pm25_bin <- factor(
  berlin_data$pm25_bin,
  levels = levels(train_data$pm25_bin)
)

```

## Build the XGBoost model for Berlin test data (KB)
```{r}
# Build model matrix using same formula as training
berlin_matrix <- model.matrix(
  avg_chip_seconds ~ .,
  data = berlin_data %>% select(-year, -n)
)

# Drop intercept column
berlin_matrix <- berlin_matrix[, -1]


# Align Berlin matrix columns with training matrix
train_matrix <- model.matrix(
  avg_chip_seconds ~ .,
  data = train_data %>% select(-year, -n)
)
...
# Keep only columns seen during training, in same order
berlin_matrix <- berlin_matrix[, colnames(train_matrix), drop = FALSE]
...
# Evaluate Berlin performance

rmse_berlin <- rmse(berlin_data$avg_chip_seconds, pred_berlin)
mae_berlin  <- mae(berlin_data$avg_chip_seconds, pred_berlin)

rss_berlin <- sum((berlin_data$avg_chip_seconds - pred_berlin)^2)
